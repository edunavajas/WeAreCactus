...
describe('/account/password', () => {
  ...
  it('requires new password', () => {
    cy.get(newPasswordSelector).should('have.class', classInvalid).type('test').should('have.class', classValid);
    cy.get(newPasswordSelector).clear();
  });

  it('requires confirm new password', () => {
    cy.get(newPasswordSelector).type('test');
    cy.get(confirmPasswordSelector).should('have.class', classInvalid).type('test').should('have.class', classValid);
    cy.get(newPasswordSelector).clear();
  ...
  it('should fail to update password when using incorrect current password', () => {
    ...
    cy.get(newPasswordSelector).type('test');
    cy.get(confirmPasswordSelector).type('test');
